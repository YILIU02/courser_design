        ...
        <div class="project">
            <el-card>
                <div class="header">
                    ...
                    <div class="search">
                        <el-input style="max-width: 600px" placeholder="请输入编号/项目名/负责人" clearable v-model="store.proName"
    ...
    <el-dialog v-model="add" title="添加项目" width="500" draggable>
        ...
        <el-form ref="addFormRef" :model="addForm" :rules="addFormRules">
            ...
            <el-form-item label="项目名" prop="projectName" required>
                <el-input type="input" placeholder="请输入添加项目名" class="dialog-input" v-model="addForm.projectName" />
            </el-form-item>

            <!-- 项目描述 -->
            <el-form-item label="项目描述" prop="projectDesc">
                <el-input type="input" placeholder="请输入项目描述" class="dialog-input" v-model="addForm.projectDesc" />
            </el-form-item>

            <!-- 负责人 -->
            <el-form-item label="负责人" prop="projMan" required>
                <el-select placeholder="部门员工" class="dialog-input" v-model="addForm.projMan">
                    <el-option v-for="(v, i) in hstore.deptempList" :label="v.empName" :value="v.id" />
                </el-select>
            </el-form-item>
        </el-form>
...
import { Search, CirclePlus, Management } from '@element-plus/icons-vue'
import { ElMessage, ElMessageBox } from "element-plus";
import api from '../../untils/api/project'
import hapi from '../../untils/api/human'
onMounted(async () => {
    api.getAll().then(async ({ data }) => {
        ...
        // 更新存储
        store.proList = projectsWithManNames;
...
const currentPageProjects = computed(() => {
    ...
    return store.showList.slice(start, end)
})
...
const showDetail = async (pro) => {
    ...
    const manName = await hapi.getById(pro.projMan).then(({ data }) => { return data.empName });
    // 3. 并行获取所有员工姓名（添加空值检查）
...
const saveEmployee = () => {
    ...
    api.editPro({
        ...
        // 更新当前显示的项目数据
        const index = store.showList.findIndex(v => v.id === updatedProject.id)
        if (index !== -1) {
            store.showList[index] = updatedProject
        }

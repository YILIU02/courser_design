    ...
    <div class="main">
        <el-card class="ecard">
            <div class="search" style="padding: 0.1em;">
                ...
                <div class="choice"
                    style=" display: flex;justify-content: space-between; padding-right: 2em;">
                    <div style="display: inline-block;">
                        <el-date-picker v-model="date" type="date"  value-format="YYYY-MM-DD" placeholder="选择时间"  style="width: 20em;" />
                    </div>
                    <div style="display: inline-block;">
                        <el-input style="max-width: 600px;width: 30em;" placeholder="请输入工号或姓名" clearable class="input-with-select" v-model="empName">
                            <template #append>
        ...
        <el-card class="ecard">
            ...
            <div class=" history">
               <el-table :data="store.dAttendList" border style="width: 100%">
...
onMounted(async () => {
    api.getdHistory(dstore.uesrInfo.deptId).then(({data}) => {
        data.forEach((v) => {
...
const search=()=>{
    ...
    empName.value=(/^[+]{0,1}(\d+)$|^[+]{0,1}(\d+\.\d+)$/).test(empName.value)?null:empName.value;
     ...
     api.getdHistory(dstore.uesrInfo.deptId,empId,empName.value,date.value).then(({data}) => {
        data.forEach((v) => {
           v.checkIn=v.checkIn?.map((v, i) => {
                        v = v < 10 ? '0' + v : v
                        v = i == 2 ? v + ' ' : i < 3 ? v + '-' : i != 5 ? v + ':' : v
